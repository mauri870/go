...
TEXT ·CountString<ABIInternal>(SB),NOSPLIT,$0-32
	...
	CBZ	R1, done
	// Broadcast the search byte and the lane-mask into vector registers.
	// Both the 16-byte tail path and the 32-byte chunk loop use them.
	VMOV	R2, V0.B16
	MOVD	$1, R5
	VMOV	R5, V5.B16
	CMP	$0x20, R1
	...
	BHS	head
tail:
	// Process 16 bytes at a time while the tail is >= 16 bytes.
	CMP	$0x10, R1
	BLO	tail_scalar
	VLD1.P	(R0), [V1.B16]
	...
	B	tail
tail_scalar:
	// Work with remaining tail shorter than 16 bytes.
	CBZ	R1, done
	MOVBU.P	1(R0), R5
	SUB	$1, R1, R1
	CMP	R2.UXTB, R5
